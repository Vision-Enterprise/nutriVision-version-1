...
function fuzzyMatch(wordRaw, keywords) {
  ...
  for (const kw of keywords) {
    ...
    if (longer >= 4 && levenshtein(w, k) <= Math.floor(longer * 0.3)) return true;
  }
  return false;
}
...
function detectColumnAnchors(headerLine) {
  ...
  for (const c of cells) {
    ...
    // Always add column to preserve spatial grid boundaries
    const finalCat = bestCat || `unknown_${xCenter}`;
    columns.push({ category: finalCat, xCenter, xLeft: xL, xRight: xR, headerText: combined });
    if (bestCat) used.add(bestCat);
  }
...
function parseDataLine(line, columns) {
  ...
  for (const word of line.words) {
    ...
    // Allow assignment even to unknown columns, but we just don't extract them
...
export function parseTableData(data) {
  ...
  if (headerFound) {
    ...
    return { 
      rows: [{ productName, qty, expDate, deliveryDate: '' }], 
      columns: [], 
      headerFound: false, 
      headerText: '', 
